...
public class Level {
    ...
    public Iterator<MapItem> getMapItemListIterator() {
        return new Iterator<MapItem>() {
            ...
            public boolean hasNext() {
                return index < mapItemList.size();
            }

            @Override
            public MapItem next() {
                MapItem mapItem = mapItemList.get(index);
                index++;
                return mapItem;
            }
        };
    ...
    public MapItem getMapItem(Position position) throws NullPointerException {
        ...
        for (MapItem mapItem : this.mapItemList) {
            if (mapItem.getPosition().equals(position)) return mapItem;
        }
        return null;
    }
    ...
    public void addMapItem(MapItem item) {
        if (item != null) {
            mapItemList.add(mapItemList.size(), item);
        }
    }
    ...
    public Blinky getBlinky() {
        Optional<Ghost> first = getGhostList().stream()
                .filter(ghost -> ghost.getClass() == Blinky.class)
                .findFirst();
        return (Blinky) first.orElse(null);
    }
    ...
    public boolean hasWon() {
        for (MapItem item : getMapItemList()) {
            if ( (item instanceof Dot || item instanceof Energizer) &&
                    !((Pickable) item).isPicked()) return false;
        }
        return true;
    }
